...
import { deleteReviewLink, updateReviewLink } from "@/server/review-links";
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const updated = await updateReviewLink(ctx.venueId, params.id, body);
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (!can(ctx.role, "manage_venue")) return NextResponse.json({ error: "forbidden" }, { status: 403 });
  try {
    await deleteReviewLink(ctx.venueId, params.id);
